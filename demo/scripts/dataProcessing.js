...
async function dataProcessing() {
    ...
    let cuTextFile = await getTextFile(`./resources/${subject}${block}${channel}/text/test_test_UASPCH-XX${subject}-ih.mlf`);
    let frameIndexTextFile = await getTextFile(`./resources/${subject}${block}${channel}/text/${subject}_${block}_frame_index.txt`);
    let googleTextFile = await getTextFile(`./resources/${subject}${block}${channel}/text/googleoutput_${subject}${block}${channel}.txt`);
    let humanTextFile = await getTextFile(`./resources/${subject}${block}${channel}/text/humanoutput_${subject}${block}.txt`);
    ...
    const googleArr = googleToArr(googleTextFile);
